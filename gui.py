...
import re
import tkinter
from tkinter import Listbox, StringVar, ttk, Text, font as tkFont, filedialog, Scale, IntVar, messagebox
import sv_ttk
import counts
import config
import process
...
def CreateScreenProcessingDisplay(root, tab):
    ...
    library_tables_file_path = (
        "/Users/rashid/Documents/Scripts/screen-processing/demo/data/library_tables"
    )
    test_mode = False
    ...
    def check_params(
        fasta_file_path, out_file_path, seq_file_names, library_tables_file_path
    ):
        if (
            fasta_file_path is None
            or out_file_path is None
            or seq_file_names is None
            or library_tables_file_path is None
        ):
    ...
    def set_out_file_path():
        global fasta_file_path, out_file_path, seq_file_names, library_tables_file_path
    ...
    def start_analysis():
        args = {
            ...
            "library_tables_file_path": library_tables_file_path,
        }
        # initial counts
    ...
    tab.frame_left.button_library_tables_file_path.grid(
        row=4, column=0, sticky="nsew", pady=5, columnspan=2
    )
...
def CreateContentMarkingDisplay(root, tab):
    ...
    # needed functions for selections
    ...
    def scan_directory_file_path():
        global folder_path, counts_files, counts_files_obj
        counts_files = [fileName for fileName in os.listdir(folder_path) if fileName.endswith(".counts")]
    ...
    def submit_counts_files():
        global counts_files_obj
        print(tab.frame_left.list_count_files.children)
        print(counts_files_obj) # dev - remove later
        return
    ...
    tab.frame_left.button_upload.grid(row=2, column=0, sticky="nsew", columnspan=2)
    ...
    def render_counts_files():
        ...
        if counts_files_obj is not None and len(counts_files_obj) > 0:
            ...
            tab.frame_left.list_count_files.grid(row=5, column=0, sticky="nsew", pady=5, padx=5, columnspan=2, rowspan=(len(counts_files_obj)))
                
            for idx, obj in enumerate(counts_files_obj):
                # create a label for the specific count
                tab.frame_left.list_count_files.label_file_name = ttk.Label(tab.frame_left.list_count_files, text=obj["file_name"])
                tab.frame_left.list_count_files.label_file_name.grid(row=(4 + idx + 1), column=0, sticky="nsew", pady=5, padx=5)
                
                tab.frame_left.list_count_files.label_condition = ttk.Entry(tab.frame_left.list_count_files, width=15, textvariable=obj["condition"])
                tab.frame_left.list_count_files.label_condition.grid(row=(4 + idx + 1), column=1, sticky="nsew", pady=5, padx=5)
                
                tab.frame_left.list_count_files.label_treatment = ttk.Entry(tab.frame_left.list_count_files, width=15, textvariable=obj["replicate_id"])
                tab.frame_left.list_count_files.label_treatment.grid(row=(4 + idx + 1), column=2, sticky="nsew", pady=5, padx=5)
            ...
            tab.frame_left.list_count_files.button_submit.config(
                style="Accent.TButton", state=tkinter.NORMAL, command=submit_counts_files
            )
            tab.frame_left.list_count_files.button_submit.grid(row=(4 + len(counts_files) + 1), column=0, sticky="nsew")
